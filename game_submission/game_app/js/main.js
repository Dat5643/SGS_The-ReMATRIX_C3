(() => {
  ...
  // State
  const state = {
    screen: "landing", // landing | menu | quiz | results
  ...
  function openModal(title, contentHTML, actions = [{ label: 'Close', variant: 'primary', onClick: closeModal }]) {
    ...
    actions.forEach(a => {
      const btn = el('button', `btn ${a.variant === 'primary' ? 'btn-primary' : a.variant === 'outline' ? 'btn-outline' : 'btn-ghost'}`);
      btn.textContent = a.label;
      btn.addEventListener('click', () => {
        try { a.onClick ? a.onClick() : closeModal(); } catch(_) {}
      });
  ...
  function saveHistory(arr) {
    try { localStorage.setItem(LS_KEY_HISTORY, JSON.stringify(arr)); } catch (_) {}
  }
  ...
  function clearHistory() {
    try { localStorage.removeItem(LS_KEY_HISTORY); } catch (_) {}
  }
  function historyContent() {
    const hist = loadHistory();
    const fmt = (ms) => new Date(ms).toLocaleString();
    if (!hist.length) return '<p>No game history yet.</p>';
    return '<div class="history-list">' + hist.map((h, i) => {
      const dur = `${Math.floor((h.durationSec||0) / 60)}m ${(h.durationSec||0) % 60}s`;
      const corr = typeof h.correct === 'number' ? `${h.correct}/${h.total} correct • ` : '';
      return `
        <div class="row" style="justify-content:space-between;gap:.75rem">
          <div>
            <div><strong>#${i + 1}</strong> • ${fmt(h.startedAt || h.endedAt || Date.now())}</div>
  ...
  function showHistory() {
    openModal('Play History', historyContent(), [
      ...
      { label: 'Clear History', variant: 'outline', onClick: () => { clearHistory(); closeModal(); } },
    ]);
  }
  ...
  async function loadData() {
    ...
    const items = itemsRaw.map(x => ({
      ...
      source: x.source || ''
    })).filter(x => (x.media_type === 'image' || x.media_type === 'video') && x.url);
  ...
  function toResults() {
    ...
    addHistory({ startedAt, endedAt, durationSec, score: state.score, total: totalQ, correct });
  ...
  function renderMenu() {
    ...
    qs('#btn-landing', card).addEventListener('click', () => { state.screen = 'landing'; render(); });
  }
  ...
  function renderLanding() {
    ...
    const goMenu = () => { state.screen = 'menu'; render(); };
  ...
  function renderQuiz() {
    ...
    if (q.type === 'single') {
      ...
      const leftMedia = q.left.media_type === 'video'
      ...
      const rightMedia = q.right.media_type === 'video'
    ...
    if (q.type === 'single') {
      ...
      const checkLayout = () => {
        ...
        const dim = (el) => {
          if (el.tagName === 'VIDEO') {
            const w = el.videoWidth || 0; const h = el.videoHeight || 0; return [w, h];
          }
          const w = el.naturalWidth || 0; const h = el.naturalHeight || 0; return [w, h];
        };
      ...
      if (mediaEls.length === 2) {
        mediaEls.forEach(el => {
          if (el.tagName === 'VIDEO') {
            if (el.readyState >= 1) setTimeout(checkLayout, 0); // metadata available
            else el.addEventListener('loadedmetadata', checkLayout, { once: true });
          } else {
  ...
  window.addEventListener('keydown', (e) => {
    if (state.screen === 'landing') {
      if (e.key === 'Enter') { state.screen = 'menu'; render(); }
      return;
    }
